...
Notes
    Added the noSnap parameter to the snapParameters class
    wordList to tell snappyHexMesh which patches to NOT snap
    Usage:
	noSnap (<names of patches within "" separated by space>)
    Example:
	noSnap ("buildings" "roads"); //does not snap on patches "buildings" & "roads"
	noSnap (); //if snap option is "on", then all patches will be snapped	

Version History
    v1: 06 Nov 2019 - initial
    v2: 03 Feb 2020 - detailed comments & cleanups
...
)
{
    ...
    // Section added by VSGR: 
    //		v1: 06 Nov 2019 - Initial edits
    //		v2: 03 Feb 2020 - Detailed comments 
    //				  Making newAdaptPatchIDs & adaptPatchIDs consistent

    // Look for the entries in the snapParams dictionary
    // Entries only to say which patches NOT to be snapped
    // Assumes that you have at least one patch that you want snapped
    // If no patch to be snapped, easier solution:
    //		Switch off snap in snappyHexMeshDict :)

    labelList noSnappingPatches;
    noSnappingPatches.setSize(snapParams.noSnap().size());
    for(int i=0;i<noSnappingPatches.size();i++)
    {
	noSnappingPatches[i] = mesh.boundaryMesh().findPatchID(snapParams.noSnap()[i]);
    }

    labelList newAdaptPatchIDs;
    newAdaptPatchIDs.setSize(adaptPatchIDs.size()-noSnappingPatches.size());
    
    label newAdaptCounter = 0;
    for(int j=0;j<adaptPatchIDs.size();j++)
    {
	if(findIndex(noSnappingPatches,adaptPatchIDs[j])==-1)
		newAdaptPatchIDs[newAdaptCounter++] = adaptPatchIDs[j];
    }
    ...
    // Repatch faces according to nearest. Do not repatch baffle faces.
    {
        ...
        repatchToSurface(snapParams, newAdaptPatchIDs, duplicateFace);
    }
